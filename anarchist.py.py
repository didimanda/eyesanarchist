...
from src.osintanarchist import osintanarchist
import argparse
...
def signal_handler(sig, frame):
    pc.printout("\nLiving the dark, Serving the light\n", pc.RED)
    sys.exit(0)
...
def _quit():
    pc.printout("Living the dark, Serving the light\n", pc.RED)
    sys.exit(0)
...
parser = argparse.ArgumentParser(description='Osintanarchist is a OSINT tool on Instagram. It offers an interactive shell '
...
api = osintanarchist(args.id, args.file, args.json, args.command, args.output, args.cookies)
